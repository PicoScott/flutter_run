...
void main(List<String> args) {
  ...
  assert(equals(unmodifiableList, list));
  // unmodifiableList.add(3); // List 不可修改: Unsupported operation: Cannot add to an unmodifiable list

  var numList = <num>[1, 2, 3];
  // var intList = numList as List<int>; // 无法进行类型转换 'List<num>' is not a subtype of type 'List<int>' in type cast
  var castList = List.castFrom<num, int>(numList);
  print(numList.runtimeType);
  print(castList.runtimeType);

  // assert(list.runtimeType is List<int>);
  var result = list.cast<num>();
  // assert(result.runtimeType is List<num>);
//  print(result);
...
//  print(castList);
  ...
  assert(equals(setLengthList, [1, 2, 3, null, null]));
  // 将 setLengthList 的长度设置为 2，移除尾部溢出元素
  setLengthList.length = 2;
  assert(equals(setLengthList, [1, 2]));

  var addList = [1, 2, 3];
  addList.add(4); // 在 addList 尾部添加元素 4
  assert(equals(addList, [1, 2, 3, 4]));

  var addAllList = [...list];
  addAllList.addAll(list); // 将 list 添加到 addAllList 的尾部
  assert(equals(addAllList, [...list, ...list]));

  var reverseList = list.reversed;
  assert(equals(reverseList, [3, 2, 1]));

  var unsortList = [2, 1, 3];
  unsortList.sort(); // 将 unsortList 按照给定比较方式(可选参数)排序
  assert(equals(unsortList, [1, 2, 3]));
...
//  print(unshuffleList);
  ...
  // 根据给定的条件在 firstWhereList 查找第一个符合的元素
  var firstWhere = firstWhereList.firstWhere((element) => element == 2);
  print(firstWhere);
  assert(firstWhere == 2);
  var firstWhereNotFound =
      firstWhereList.firstWhere((element) => element == 5, orElse: () => -1);
